/* 
Basato su: P_2_3_1_02 — Segue licenza originale.
...
void setup() {
  ...
  background(0);// cambio colore da bianco a nero
  smooth();
  cursor(CROSS);
}
...
void draw() {
  if (mousePressed) {
    ...
    if (lineModule != null) {
      shape(lineModule, 10, 10, lineModuleSize, lineModuleSize); // da 0 a 10
    } 
    else {
      line(0, 0, lineModuleSize, lineModuleSize);
    }
    angle = angle+10 + angleSpeed; // aggiunto + 10 a angle: ho raggi più radi
    popMatrix();
  }
}
...
void mousePressed() {
  ...
  lineModuleSize = random(50, 200); // cambiato da 50,160 a 50,200
...
void keyReleased() {
  if (key == DELETE || key == BACKSPACE) background(0); // richiamo lo sfondo nero
  ...
  if (key=='d' || key=='D') {
    angle = angle + 30; // da 180 a 30 stringo l'angolo
    angleSpeed = angleSpeed * -1;
  }

  // r g b alpha  
  //aggiunta 'r' , dato un range specifico a Hue
  if (key == 'r') col = color(random(250, 255), random(255), random(255), random(80, 150)); 


  //default colors from 1 to 4 
  if (key == '1') col = color(181, 157, 0, 100);
  if (key == '2') col = color(0, 130, 164, 100);
  if (key == '3') col = color(100, 35, 129, 100); //cambio viola
...
void keyPressed() {
  if (keyCode == UP) lineModuleSize += 10; // cambiato da 5 a 10 per fare più incremento
  if (keyCode == DOWN) lineModuleSize -= 10; 
